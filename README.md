...
│   ├─ dashboard/report        (JMeter HTML 대시보드 리포트 폴더)
│   ├─ results/                (실행 결과 데이터)
...
| 사용자 수 | Throughput (/min) | TPS | Avg Latency (ms) | 비고 |
| :--- | :--- | :--- | :--- | :--- |
| **10명** | `45.8` | `0.76` | `154` | |
| **30명** | `124.6` | `2.08` | `140` | |
| **50명** | `215.5` | `3.59` | `132` | |
| **70명** | `313.9` | `5.23` | `131` | |
| **100명** | `433.7` | `7.23` | `139` | **Max** |

- **목표 달성 여부**
  1. **Error Rate**: 100명 부하 기준 `0.62%` 기록 (목표치 1% 미만 달성)
  2. **평균 Latency**: 모든 구간에서 `131ms` ~ `154ms` 유지 (목표치 5,000ms 이내 달성)

### 7. 모니터링 및 병목 현상 분석 (Analysis)

- **상관관계 및 임계 지점 분석**
  1. **TPS와 Latency 상관관계**: 사용자 수가 10명에서 100명으로 증가함에 따라 TPS는 선형적으로 증가(`0.76` → `7.23`)하였으나, 평균 Latency는 부하 증가와 무관하게 `130~150ms`대를 안정적으로 유지하는 모습을 보였습니다.
  2. **임계 지점 및 변곡점 도출**: 100명 동시 접속 환경에서도 Latency의 급격한 상승(변곡점)이나 TPS의 정체/하락(임계 지점)이 관찰되지 않았습니다. 다만, 일부 API에서 응답 시간 편차가 확인되어 지속적인 성능 튜닝이 권고됩니다.

- **발견된 에러 및 병목 구간 (User 100명 기준)**

| 발생 구간 | 에러 유형 (Error Code) | 현상 및 원인 분석 |
| :--- | :--- | :--- |
| **Auth** | `422 Unprocessable Entity` | 일부 인증 요청 시 간헐적 발생. 동일 조건 재실행 시 미발생하는 등 재현 안정성이 낮아 개발 환경 기준 추가 검증 필요. |
| **3. 시험 시작** | `java.net.SocketException` | 일부 요청에서 서버 연결이 조기 종료되는 소켓 닫힘(Socket closed) 오류 확인. 최대 부하 시 커넥션 관리 및 서버 연결 안정성에 대한 점검 권장. |
